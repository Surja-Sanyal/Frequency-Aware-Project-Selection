...
MAX_FREQ                                = 1000                                   #Max tourist visit frequency
...
def read_csv():
    ...
    load, store, cmax, items, max_price, min_price, max_freq = DATA_LOAD_LOCATION, DATA_STORE_LOCATION, COORDINATE_MAX, ITEMS, MAX_PRICE, MIN_PRICE, MAX_FREQ
    locations = []
    ...
    item_price_list, tourist_freq_list = [], []
    for element in locations:

        item_price_list = item_price_list + [[element%items, (random.choice(locations))%max_price]]
        if (element != 0 and element not in tourist_freq_list):
            tourist_freq_list += [element]
    ...
    #   Write data  #
    pickle.dump(tourist_freq_list, open(store + "_tourist_freq.pickle", "wb"))
